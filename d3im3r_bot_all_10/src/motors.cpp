...
static bool last_dir_left_forward  = true;
static bool last_dir_right_forward = true;
...
static inline uint32_t duty_to_ledc(float duty_percent)
{
    ...
    if (duty_percent >= 100.0f) return (1 << PWM_RES_BITS) - 1;
...
)
{
    ...
    bool in1 = dir_forward ? forward_in1 : !forward_in1;
    bool in2 = dir_forward ? forward_in2 : !forward_in2;

    // Solo aplicar dead-time de 300us si la dirección invierte su sentido
    if (dir_forward != last_dir_store) {
        ledcWrite(pwm_channel, 0);
        delayMicroseconds(300);
        last_dir_store = dir_forward;
    }
...
void motors_set_left(float cmd_percent)
{
    ...
    // LEFT: avance = IN1 HIGH (true), IN2 LOW (false)
    motor_write(
        cmd,
        MOTOR_LEFT_IN1_PIN,
        MOTOR_LEFT_IN2_PIN,
...
void motors_set_right(float cmd_percent)
{
    ...
    // RIGHT: avance = IN1 LOW (false), IN2 HIGH (true)
    motor_write(
        cmd,
        MOTOR_RIGHT_IN1_PIN,
        MOTOR_RIGHT_IN2_PIN,
...
void motors_update()
{
    ...
    if (last_pwm_left != 0.0f && (now - last_left_cmd_ms) > MOTOR_TIMEOUT_MS) {
        motors_set_left(0.0f);
    }

    if (last_pwm_right != 0.0f && (now - last_right_cmd_ms) > MOTOR_TIMEOUT_MS) {
