...
bool uros_init()
{
    ...
    if (rclc_node_init_default(&node, "d3im3r_encoders", "", &support) != RCL_RET_OK) return false;
    ...
    if (rclc_executor_add_subscription(
            &executor, &load_left_sub,
            &load_left_msg, &load_left_callback,
            ON_NEW_DATA) != RCL_RET_OK) return false;

    if (rclc_executor_add_subscription(
            &executor, &load_right_sub,
            &load_right_msg, &load_right_callback,
            ON_NEW_DATA) != RCL_RET_OK) return false;
