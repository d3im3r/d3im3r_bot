...
// cmd < 0  => retroceso
//
// AJUSTAR forward_in1 / forward_in2 si gira al revés
// =====================================================
void motors_set_left(float cmd_percent)
{
    float cmd = clamp_and_apply_deadband(cmd_percent);

    // Ejemplo actual: avance = IN1 LOW, IN2 HIGH
    motor_write(
        cmd,
        MOTOR_LEFT_IN1_PIN,
        MOTOR_LEFT_IN2_PIN,
...
// =====================================================
// Set rueda derecha
// AJUSTAR si el sentido real no coincide
// =====================================================
void motors_set_right(float cmd_percent)
{
    float cmd = clamp_and_apply_deadband(cmd_percent);

    // Ejemplo actual: avance = IN1 HIGH, IN2 LOW
    motor_write(
        cmd,
        MOTOR_RIGHT_IN1_PIN,
        MOTOR_RIGHT_IN2_PIN,
